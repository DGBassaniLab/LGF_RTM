...
*Misuse of the reduced form of Tim Cole's formula for catch-up: Z2 - rZ1 (as per Cameron et al. 2005)
di haz3_mean_stunted*rho_haz3_haz6
di haz6_mean_stunted-(haz3_mean_stunted*rho_haz3_haz6)

*Further misuse of the reduced form of the Cole formula by using correlation only in children-stunted-at-3 months group
...
graph export "StuntR_Fig1a.png", as(png) replace width(5000) height(3000)

******************************
*Part 1B - with faltering
****************************** 
clear
set obs 100000
set seed 998877
...
gr export "StuntR_Fig1_panel.png", as(png) width(3000) height(5000) replace
gr export "StuntR_Fig1_panel.eps", as(eps) replace
...
twoway (scatter haz agejitter if tag == 0, mcolor(gs13) msize(tiny) msymbol(circle)) ///
       ...
       (scatter haz agejitter if tag2 == 1, mcolor(dkgreen) msize(tiny) msymbol(circle)), ///
       legend(order(2 3) label(2 "HAZ<-1 at 3 months") label(3 "HAZ>+1 at 3 months") pos(6) /// 
	   col(2) size(2) region(lcolor(black) lwidth(0.08))) ///
...
graph export "StuntR_Fig2.png", as(png) replace width(4000) height(3000)
...
clear all
**Increased number of observations to produce stable estimates even with random seed (kept seed fixed to ensure replication)
set obs 100000
set seed 08252107
...
* Tag obs. if HAZ at 3-mos less than -2 but greater than -2.5
...
graph export "StuntR_Fig3.png", as(png) replace width(3000) height(3000)
